(function (root, factory) {
  ...
  function toCountryState(definition) {
    ...
    return {
      ...
      fiscalBalance: 0, tradeBalance: 0,
  ...
  function createGame(countryId, leaderId, seed) {
    ...
    const game = {
      ...
      countries, eventsEnabled: false, activity: [], history: [], gameOver: null
    };
  ...
  function productionAndDemand(country, state) {
    ...
    Object.keys(supply).forEach((key) => { supply[key] = Math.max(0.01, supply[key]); demand[key] = Math.max(0.01, demand[key]); });
  ...
  function simulateMarkets(state) {
    ...
    DATA.commodities.forEach((commodity) => {
      const id = commodity.id;
      const exporters = Object.values(state.countries).map((country) => ({ country, available: Math.max(0, snapshots[country.id].supply[id] - snapshots[country.id].demand[id]) })).filter((entry) => entry.available > 0.01);
      const importers = Object.values(state.countries).map((country) => ({ country, needed: Math.max(0, snapshots[country.id].demand[id] - snapshots[country.id].supply[id]) })).filter((entry) => entry.needed > 0.01).sort((a, b) => b.needed - a.needed);
      importers.forEach((importer) => {
        ...
        for (const exporter of exporters) {
          ...
          const actual = quantity * clamp((importer.country.relations[exporter.country.id] || 50) / 70, 0.55, 1.2);
          const value = actual * state.market.prices[id] * 0.72;
          importer.needed -= actual; exporter.available -= actual;
          importer.country.tradeBalance -= value; exporter.country.tradeBalance += value;
  ...
  function advanceProjects(country, state) {
    ...
    country.projects = country.projects.slice(-40);
  }
  ...
  function simulateCountry(country, state) {
    ...
    const growth = clamp(country.baseGrowth + educationDrive + industryDrive + infrastructureDrive + tradeImpulse - inflationDrag - unemploymentDrag, -12, 14);
    ...
    const structuralInflation = clamp(2.4 + Math.max(0, country.spendingTarget + subsidyBurden - revenueRate) * 0.34 - Math.max(0, country.productivity - 80) * 0.008, 1, 35);
    ...
    const happinessTarget = clamp(52 + (country.education - 65) * 0.13 + (country.health - 65) * 0.14 + (country.infrastructure - 60) * 0.1 + (country.housing - 60) * 0.14 - Math.max(0, country.unemployment - 6) * 0.55 - Math.max(0, country.inflation - 5) * 0.17, 10, 92);
  ...
  function evaluateGame(state) {
    ...
    else if (state.tick >= MONTHS_LIMIT) {
  ...
  function ensureCountryV2(country, definition) {
  ...
  function migrate(candidate) {
    ...
    if (state.version !== 1 && state.version !== 2) throw new Error("La versión de la partida no es compatible.");
    ...
    DATA.countries.forEach((definition) => {
      ...
      else ensureCountryV2(state.countries[definition.id], definition);
    });
    ...
    state.eventsEnabled = false; state.version = SAVE_VERSION; state.gameOver = state.gameOver || null;
  ...
  function validateSave(candidate) {
    ...
    if (candidate.version !== 1 && candidate.version !== SAVE_VERSION) throw new Error("La versión de la partida no es compatible.");
  ...
  return {
    SAVE_VERSION, MONTHS_LIMIT, createGame, advanceTick, applyAllocations, queueConstruction,
