const CACHE_NAME = "pulso-global-v7.10.0";
const ASSETS = [
  "./",
  "./index.html",
  "./styles.css",
...
async function assetList() {
  ...
  if (!response.ok)
    throw new Error("No se pudo cargar el manifiesto de iconos");
...
self.addEventListener("install", (event) => {
  event.waitUntil(
    (async () => {
      const [cache, icons] = await Promise.all([
  ...
  event.waitUntil(
    (async () => {
      ...
      await Promise.all(
        keys
          .filter(
            (key) => key.startsWith("pulso-global-") && key !== CACHE_NAME,
          )
