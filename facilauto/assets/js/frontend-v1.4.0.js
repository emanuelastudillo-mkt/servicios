/**
 * FACIL AUTO — Frontend presentation v1.4.0
...
function currentShareData() {
  ...
  return {
    ...
    text: lines.join('\n'),
    url: new URLSearchParams(location.search).get('share_url') || `${location.origin}${location.pathname}`
  };
}
...
function setupEmbedMode() {
  ...
  document.body.classList.add('fa-embed-mode');

  const style = document.createElement('style');
  style.id = 'fa-embed-style';
  style.textContent = `
    html,body{background:#121212!important}
    body.fa-embed-mode .topbar,
    body.fa-embed-mode .hero,
    body.fa-embed-mode .method-section,
    body.fa-embed-mode .sources-section,
    body.fa-embed-mode .footer{display:none!important}
    body.fa-embed-mode main{display:block!important}
    body.fa-embed-mode .calculator-section{padding:46px 0 56px!important}
    body.fa-embed-mode .calculator-section .section-title{margin-top:0}
    body.fa-embed-mode .results-section{background:#f3f1eb;padding:56px 0 70px}
    body.fa-embed-mode .shell{width:min(1180px,calc(100% - 36px))}
    @media(max-width:700px){
      body.fa-embed-mode .calculator-section{padding-top:30px!important}
      body.fa-embed-mode .shell{width:min(100% - 24px,1180px)}
    }
  `;
  document.head.appendChild(style);

  const km = params.get('km');
  if (km && document.querySelector('#km')) document.querySelector('#km').value = km;

  const security = params.get('security');
  if (security && document.querySelector('#security-zone')) {
    const el = document.querySelector('#security-zone');
    if ([...el.options].some(o => o.value === security)) el.value = security;
  }

  const jurisdiction = params.get('jurisdiction');
  if (jurisdiction && document.querySelector('#jurisdiction')) {
    const el = document.querySelector('#jurisdiction');
    if ([...el.options].some(o => o.value === jurisdiction)) el.value = jurisdiction;
  }

  const focus = params.get('focus') || 'valuation';
  ...
  function applyFocus() {
    ...
    document.querySelectorAll('.result-strip').forEach(el => { el.open = false; });
    ...
    const target = document.querySelector(map[focus] || '.valuation-strip');
    if (target) {
      target.open = true;
  ...
  document.querySelector('#vehicle-form')?.addEventListener('submit', () => {
  ...
  const sendHeight = () => {
    ...
    parent.postMessage({type:'facilauto:embed-height', height}, '*');
  };
